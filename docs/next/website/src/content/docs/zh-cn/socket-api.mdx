...
| 工作流 | `workflow.run`、`workflow.list`、`workflow.get`、`workflow.pause`、`workflow.resume`、`workflow.cancel`、`workflow.delete` |
...
工作流是一个 `.aflow.json` 文件：带 `after` 依赖边的命名节点列表。`workflow.run {path}` 解析并校验文件、创建专属 workspace、调度 DAG。节点类型：`agent`（claude-code / codex / custom 运行时，按节点绑定供应商档案）、`prompt_template`（纯文本渲染）、`image_gen`（经绑定档案的 OpenAI 兼容 images 端点）。
...
`workflow.pause` 是软停止：在跑的节点跑完、不派发新节点。`workflow.resume` 重新读取工作流文件并通过内容哈希缓存跳过未变节点——暂停中编辑文件（增删改节点）后 resume，只有受影响的节点重跑。`workflow.cancel` 杀掉在跑节点并保留缓存；`workflow.delete` 删除 run 目录与 workspace。run 持久化在状态目录（`workflows/runs/<run_id>/`），保留最新 `workflow.keep_runs` 个。`workflow.max_concurrent_agents` 限制所有 run 的并发 agent 节点总数。custom 命令经平台 shell 执行：Unix 用 POSIX sh，Windows 用 PowerShell 5.1（不支持 `&&`，请用 `;`）。
