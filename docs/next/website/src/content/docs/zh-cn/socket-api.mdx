...
| 供应商 | `provider.list`、`provider.get`、`provider.create`、`provider.update`、`provider.delete`、`provider.presets`、`provider.test`、`provider.models.fetch`、`provider.reveal` |
| 工作流 | `workflow.run`、`workflow.list`、`workflow.get`、`workflow.pause`、`workflow.resume`、`workflow.cancel`、`workflow.delete`、`workflow.update` |
...
供应商档案为工作流节点绑定“用哪个模型、走哪个 Key、连哪个地址”。档案由 `{protocol, base_url, api_key, models}` 组成；同一供应商可以建多条档案（每个 Key 一条），`protocol` 区分线协议：`openai-compat`、`anthropic`、`gemini`。Base URL 只填到根（例如 `/v1`），端点由服务端拼接。

档案保存在用户全局的 `providers.json`（0600）中，跨会话共享。API 响应中的 Key 始终掩码显示（`sk-***cdef`）；只有 `provider.reveal` 返回明文。更新语义为“省略即保留”——不传 `api_key` 就保留已存 Key。

`provider.test` 发送一次 1 token 的最小对话请求（无可见模型时降级为模型端点探测），并分类失败原因：401/403 视为认证失败、404 视为端点配错、429 视为限流。`provider.models.fetch` 拉取模型列表并合并进档案（手工条目保留、可见性按 id 继承，上限 500 个）。两者都在后台 curl 子进程中执行（10–15 秒超时）并异步应答；API Key 绝不出现在 curl 命令行中。

## 工作流

工作流是一个 `.aflow.json` 文件：带 `after` 依赖边的命名节点列表。`workflow.run {path}` 解析并校验文件、创建专属 workspace、调度 DAG。节点类型：`agent`（claude-code / codex / grok-build / dsh / custom 运行时，按节点绑定供应商档案）、`prompt_template`（纯文本渲染）、`image_gen`（经绑定档案的 OpenAI 兼容 images 端点）。grok-build 与 dsh 接受 openai-compat 档案，并在每节点的临时 HOME（`GROK_HOME` / `DSH_HOME`）下运行，用户全局配置零触碰。

可见 agent 节点以带标签 tab 的形式运行在 run workspace 里；平台 shell 包装把输出镜像到 run 目录并追加 `HERDR_EXIT:<code>` 哨兵，因此退出码与输出在 pane 拆除后依然可用。不可见节点（`"visible": false`）以后台进程执行。

`workflow.pause` 是软停止：在跑的节点跑完、不派发新节点。`workflow.resume` 重新读取工作流文件并通过内容哈希缓存跳过未变节点——暂停中编辑文件（增删改节点，含已完成的节点）后 resume，只有受影响的节点重跑；error run 修好后也能 resume。`workflow.cancel` 杀掉在跑节点并保留缓存；`workflow.delete` 删除 run 目录与 workspace。`workflow.update {path, patches}` 在文件里编辑并校验节点绑定（runtime / 档案 / 模型 / 超时 / 可见性 / enabled）；空串解除可选绑定，未知的 JSON 字段原样保留。禁用节点（`"enabled": false`）及其下游以空输出端口结构性完成。run 持久化在状态目录（`workflows/runs/<run_id>/`），保留最新 `workflow.keep_runs` 个。`workflow.max_concurrent_agents` 限制所有 run 的并发 agent 节点总数。custom 命令经平台 shell 执行：Unix 用 POSIX sh，Windows 用 PowerShell 5.1（不支持 `&&`，请用 `;`）。
