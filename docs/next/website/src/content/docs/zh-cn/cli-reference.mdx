...
`provider list` 始终掩码显示 Key；`provider reveal` 返回存储的明文。`provider update` 省略 `--key` 即保留当前 Key。`provider test` 以最小请求拨测一次并回报 HTTP 状态、延迟与分类后的错误。`provider models` 拉取模型列表并合并进档案。档案按用户保存（不分会话），也可以在设置页管理。
## 工作流

运行与控制 `.aflow.json` 工作流 DAG:
...
`workflow run` 校验文件、创建 run workspace，并把 agent 节点派发为可见 tab（或 `"visible": false` 时后台进程）。`workflow pause` 让在跑节点跑完但不再派发；`workflow resume` 重读文件，只重跑配置或上游输出变化了的节点（修好后的 error run 也能 resume）。`workflow cancel` 杀掉在跑节点并保留缓存。`workflow bind` 在文件里编辑节点的单值绑定（与 TUI inspector 同字段）；`--profile` / `--model` 传空串即解除绑定。节点 prompt 用 `{{node_id.output}}` 引用上游输出；每个 agent 节点绑定自己的供应商档案（模型、Key、地址）。
## 读取来源
