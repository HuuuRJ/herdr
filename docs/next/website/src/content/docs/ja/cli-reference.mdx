...
プロバイダープロファイルは、ワークフローノード用にモデル・キー・エンドポイントを束ねます:
...
`provider list` のキーは常にマスク表示され、`provider reveal` が保存済み平文を返します。`provider update` で `--key` を省略すると現在のキーが保持されます。`provider test` は最小リクエストを 1 回送信し、HTTP ステータス・レーテンシ・分類済みエラーを報告します。`provider models` はモデル一覧を取得してプロファイルにマージします。プロファイルはユーザー単位（セッション単位ではない）で保存され、設定タブからも管理できます。
## ワークフロー

`.aflow.json` ワークフロー DAG の実行と制御:
...
`workflow run` はファイルを検証し、run ワークスペースを作成して、エージェントノードを可視タブ（または `"visible": false` でバックグラウンドプロセス）として配信します。`workflow pause` は実行中のノードを完了させつつ新しい配信を停止します。`workflow resume` はファイルを再読み込みし、設定または上流出力が変更されたノードのみ再実行します（修正後のエラー run も resume できます）。`workflow cancel` は実行中のノードを強制終了しキャッシュを保持します。`workflow bind` はノードの単一値バインド（TUI インスペクターと同じフィールド）をファイル内で編集します。空の `--profile` / `--model` 値はバインドを解除します。ノードプロンプトは `{{node_id.output}}` で上流出力を参照し、各エージェントノードは独自のプロバイダープロファイル（モデル・キー・エンドポイント）をバインドします。
## 読み取りソース
