...
| プロバイダー | `provider.list`、`provider.get`、`provider.create`、`provider.update`、`provider.delete`、`provider.presets`、`provider.test`、`provider.models.fetch`、`provider.reveal` |
| ワークフロー | `workflow.run`、`workflow.list`、`workflow.get`、`workflow.pause`、`workflow.resume`、`workflow.cancel`、`workflow.delete`、`workflow.update` |
...
プロバイダープロファイルは、ワークフローノードで「どのモデルを、どのキーで、どのエンドポイントに対して使うか」を束ねます。プロファイルは `{protocol, base_url, api_key, models}` で構成され、同一ベンダーでもキーごとに複数のプロファイルを持てます。`protocol` はワイモ形式（`openai-compat`、`anthropic`、`gemini`）を区別します。ベース URL はルート（例: `/v1`）まで指定し、エンドポイントはサーバー側で付加されます。

プロファイルはユーザー共通の `providers.json`（0600）に保存され、セッション間で共有されます。API レスポンスのキーは常にマスクされ（`sk-***cdef`）、平文を返すのは `provider.reveal` だけです。更新は省略時解釈が「保持」です - `api_key` を省略すると保存済みキーがそのまま使われます。

`provider.test` は 1 トークンの最小チャットリクエスト（表示モデルがない場合は models エンドポイントの探索にフォールバック）を送り、失敗を分類します: 401/403 は認証、404 はエンドポイント、429 はレート制限。`provider.models.fetch` はモデル一覧を取得してプロファイルにマージします（手動エントリは保持、可視性は id 単位で継承、上限 500）。どちらもバックグラウンドの curl サブプロセス（10～15 秒タイムアウト）で非同期に実行され、API キーが curl のコマンドラインに現れることはありません。

## ワークフロー

ワークフローは `.aflow.json` ファイルです: `after` 依存エッジを持つ名前付きノードリスト。`workflow.run {path}` はファイルを解析・検証し、専用ワークスペースを作成して DAG をスケジュールします。ノードタイプ: `agent`（claude-code / codex / grok-build / dsh / custom ランタイム、ノードごとにプロバイダープロファイルをバインド）、`prompt_template`（純粋なテキストレンダリング）、`image_gen`（バインドされたプロファイル経由の OpenAI 互換 images エンドポイント）。grok-build と dsh は openai-compat プロファイルを受け付け、ノードごとの一時ホーム（`GROK_HOME` / `DSH_HOME`）で実行されるためユーザーのグローバル設定は変更されません。

可視エージェントノードは run ワークスペース内のラベル付きタブとして実行されます。プラットフォームシェルラッパーは出力を run ディレクトリにミラーし、`HERDR_EXIT:<code>` センチネルを追記するため、終了コードと出力はペーン破棄後も残ります。不可視ノード（`"visible": false`）はバックグラウンドプロセスとして実行されます。

`workflow.pause` はソフトストップです: 実行中のノードは完了し、新しいノードは配信されません。`workflow.resume` はワークフローファイルを再読み込みし、コンテンツハッシュキャッシュにより変更されていないノードをスキップします — 一時停止中にファイルを編集（完了済みノードを含む追加・削除・変更）して resume すると、影響を受けたノードのみ再実行されます。エラー終了した run も修正後に resume できます。`workflow.cancel` は実行中のノードを強制終了しキャッシュを保持します。`workflow.update {path, patches}` はノードのバインド（ランタイム / プロバイダープロファイル / モデル / タイムアウト / 可視性 / enabled）をファイル内で編集・検証します。空文字列はオプションのバインドを解除し、未知の JSON フィールドは保持されます。無効化されたノード（`"enabled": false`）とその下流は空の出力ポートで構造的に完了します。run はステートディレクトリ（`workflows/runs/<run_id>/`）に永続化され、最新の `workflow.keep_runs` 件が保持されます。`workflow.max_concurrent_agents` は全 run を通じた同時実行エージェントノード数の上限です。カスタムコマンドはプラットフォームシェル（Unix は POSIX sh、Windows は PowerShell 5.1 — `&&` は使えないため `;` を使用）で実行されます。
