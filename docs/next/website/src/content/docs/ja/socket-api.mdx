...
| ワークフロー | `workflow.run`、`workflow.list`、`workflow.get`、`workflow.pause`、`workflow.resume`、`workflow.cancel`、`workflow.delete` |
...
ワークフローは `.aflow.json` ファイルです: `after` 依存エッジを持つ名前付きノードリスト。`workflow.run {path}` はファイルを解析・検証し、専用ワークスペースを作成して DAG をスケジュールします。ノードタイプ: `agent`（claude-code / codex / custom ランタイム、ノードごとにプロバイダープロファイルをバインド）、`prompt_template`（純粋なテキストレンダリング）、`image_gen`（バインドされたプロファイル経由の OpenAI 互換 images エンドポイント）。
