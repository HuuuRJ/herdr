...
Provider profiles bind a model, key, and endpoint for workflow nodes:

```bash
herdr provider list
herdr provider presets
herdr provider get <profile_id>
...
herdr provider delete <profile_id>
herdr provider test <profile_id>
herdr provider models <profile_id>
herdr provider reveal <profile_id>
```

`provider list` always masks keys; `provider reveal` returns the stored plaintext. Omit `--key` on `provider update` to keep the current key. `provider test` dials the provider once with a minimal request and reports HTTP status, latency, and a classified error. `provider models` fetches and merges the model list into the profile. Profiles are stored per user (not per session) and can also be managed from the settings tab.
## Workflows

Run and control `.aflow.json` workflow DAGs:

```bash
herdr workflow run <path.aflow.json>
herdr workflow list
herdr workflow get <run_id>
herdr workflow pause <run_id>
herdr workflow resume <run_id>
herdr workflow cancel <run_id>
herdr workflow delete <run_id>
herdr workflow bind <path.aflow.json> --node <id> [--runtime <r>] [--profile <id>] [--model <m>] [--timeout-ms <n>] [--visible|--invisible] [--enable|--disable]
```

`workflow run` validates the file, creates a run workspace, and dispatches agent nodes as visible tabs (or background processes with `"visible": false`). `workflow pause` finishes running nodes but dispatches nothing new; `workflow resume` re-reads the file and re-runs only nodes whose config or upstream outputs changed (errored runs resume too, after a fix). `workflow cancel` kills running nodes and keeps the cache. `workflow bind` edits a node's single-value bindings in the file — the same fields the TUI inspector edits; an empty `--profile` or `--model` value clears the binding. Node prompts reference upstream outputs with `{{node_id.output}}`; each agent node binds its own provider profile (model, key, endpoint).
## Read sources
