...
| Workflows | `workflow.run`, `workflow.list`, `workflow.get`, `workflow.pause`, `workflow.resume`, `workflow.cancel`, `workflow.delete` |
...
A workflow is a `.aflow.json` file: a named node list with `after` dependency edges. `workflow.run {path}` parses and validates the file, creates a dedicated workspace, and schedules the DAG. Node types: `agent` (claude-code / codex / custom runtimes, bound to a provider profile per node), `prompt_template` (pure text rendering), and `image_gen` (OpenAI-compatible images endpoint through the bound profile).
...
`workflow.pause` is a soft stop: running nodes finish, no new nodes dispatch. `workflow.resume` re-reads the workflow file and skips unchanged nodes via a content-hash cache — edit the file mid-pause (add, remove, or modify nodes), resume, and only affected nodes re-run. `workflow.cancel` kills running nodes and keeps the cache; `workflow.delete` removes the run directory and workspace. Runs persist under the state directory (`workflows/runs/<run_id>/`) with the newest `workflow.keep_runs` kept. `workflow.max_concurrent_agents` caps concurrent agent nodes across all runs. Custom commands run through the platform shell: POSIX sh on Unix, Windows PowerShell 5.1 on Windows (avoid `&&` there; use `;`).
