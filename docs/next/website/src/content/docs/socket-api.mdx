...
| Providers | `provider.list`, `provider.get`, `provider.create`, `provider.update`, `provider.delete`, `provider.presets`, `provider.test`, `provider.models.fetch`, `provider.reveal` |
| Workflows | `workflow.run`, `workflow.list`, `workflow.get`, `workflow.pause`, `workflow.resume`, `workflow.cancel`, `workflow.delete`, `workflow.update` |
...
Provider profiles bind "which model, through which key, at which endpoint" for later workflow nodes. A profile is `{protocol, base_url, api_key, models}`; the same vendor can have several profiles (one per key), and `protocol` distinguishes the wire family: `openai-compat`, `anthropic`, or `gemini`. Base URLs only go to the root (for example `/v1`); endpoints are appended by the server.

Profiles live in a user-global `providers.json` (0600) and are shared across sessions. API responses always mask the key (`sk-***cdef`); `provider.reveal` is the only method that returns the plaintext. Updates use omit-means-keep semantics - leave out `api_key` to keep the stored key.

`provider.test` sends a one-token minimal chat request (falling back to a models-endpoint probe when no visible model is set) and classifies failures: 401/403 as auth, 404 as endpoint, 429 as rate limited. `provider.models.fetch` pulls the model list and merges it into the profile (manual entries survive, visibility is inherited by id) with a 500-model cap. Both run in a background curl subprocess with a 10-15s timeout and answer asynchronously; API keys never appear in the curl command line.

## Workflows

A workflow is a `.aflow.json` file: a named node list with `after` dependency edges. `workflow.run {path}` parses and validates the file, creates a dedicated workspace, and schedules the DAG. Node types: `agent` (claude-code / codex / grok-build / dsh / custom runtimes, bound to a provider profile per node), `prompt_template` (pure text rendering), and `image_gen` (OpenAI-compatible images endpoint through the bound profile). grok-build and dsh accept openai-compat profiles and run against per-node temporary homes (`GROK_HOME` / `DSH_HOME`), so user-global config stays untouched.

Visible agent nodes run as labeled tabs in the run workspace; the platform shell wrapper mirrors output into the run directory and appends a `HERDR_EXIT:<code>` sentinel, so exit codes and output survive pane teardown. Invisible nodes (`"visible": false`) run as background processes.

`workflow.pause` is a soft stop: running nodes finish, no new nodes dispatch. `workflow.resume` re-reads the workflow file and skips unchanged nodes via a content-hash cache — edit the file mid-pause (add, remove, or modify nodes, including already-completed ones), resume, and only affected nodes re-run; errored runs resume after a fix. `workflow.cancel` kills running nodes and keeps the cache; `workflow.delete` removes the run directory and workspace. `workflow.update {path, patches}` edits node bindings in the file (runtime / provider profile / model / timeout / visibility / enabled; empty strings clear optional bindings) and validates the result — unknown JSON fields survive the edit. Disabled nodes (`"enabled": false`) and their downstream complete structurally with empty output ports. Runs persist under the state directory (`workflows/runs/<run_id>/`) with the newest `workflow.keep_runs` kept. `workflow.max_concurrent_agents` caps concurrent agent nodes across all runs. Custom commands run through the platform shell: POSIX sh on Unix, Windows PowerShell 5.1 on Windows (avoid `&&` there; use `;`).
